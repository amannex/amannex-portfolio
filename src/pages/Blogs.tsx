 ...
 * 
 * Displays a list of blog posts with category filtering.
 * Manages the active category state and re-triggers scroll animations 
 * when the category changes to ensure new elements fade in smoothly.
 */
const Blogs = () => {
  // State to track the currently selected blog category
  const [activeCategory, setActiveCategory] = useState(blogCategories[0]);

  // Filter posts based on the active category. If "All Posts" is selected, show all.
  const filteredPosts =
    activeCategory === blogCategories[0]
      ? blogPosts
      : blogPosts.filter((post) => post.category === activeCategory);

  // Effect to re-initialize IntersectionObserver for fade-up animations
  // whenever the active category changes, ensuring newly rendered blog cards animate in.
  useEffect(() => {
    ...
    <>
      ...
      <header className="page-header">
        <div className="container fade-up">
          ...
          <p className="section-subtitle" style={{ marginBottom: "1.5rem" }}>
            Exploring web development, artificial intelligence, UI/UX design,
      ...
      <section className="section section-alt">
        <div className="container">
          ...
          {(activeCategory === blogCategories[0] ||
            featuredBlogPost.category === activeCategory) && (
            <div className={`${styles["featured-post"]} fade-up`}>
              <Link
                to={featuredBlogPost.link}
                className={`${styles["featured-img-container"]} interactive-element`}
              >
              ...
              <div className={`${styles["featured-content"]}`}>
                ...
                <Link
                  to={featuredBlogPost.link}
                  className={`${styles["read-more"]} interactive-element`}
                >
          ...
          <div className={`${styles["blog-grid"]}`}>
            {filteredPosts.map((post, index) => (
              <article
                key={post.title}
                className={`${styles["blog-card"]} fade-up`}
                style={{ transitionDelay: `${index * 0.1}s` }}
              >
                <Link
                  to={post.link}
                  className={`${styles["blog-card-img"]} interactive-element`}
                >
                ...
                <Link
                  ...
                  className={`${styles["read-more"]} interactive-element`}
                >
                  Read Mode <i className="fas fa-arrow-right"></i>
                </Link>
              </article>
            ))}
          </div>
          ...
          <div className={`${styles.pagination} fade-up`}>
            <button className={`${styles["page-btn"]} interactive-element`}>
              <i className="fas fa-chevron-left"></i>
            </button>
            <button
              className={`${styles["page-btn"]} active interactive-element`}
            >
              1
            </button>
            <button className={`${styles["page-btn"]} interactive-element`}>
          ...
          <div className={`${styles["cta-section"]} fade-up`}>
            ...
            <p>
              Join the newsletter to receive my latest articles, coding
              tutorials, and design resources directly in your inbox. No spam,
              ever.
            </p>
            <form className={`${styles["newsletter-form"]}`}>
              ...
              <button
                type="submit"
                className={`${styles["newsletter-btn"]} interactive-element`}
              >
                Subscribe{" "}
                <i
